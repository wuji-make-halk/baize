...
class Wxpay extends CI_Controller
{
    ...
    public function unifiedorder()
    {
        ...
        //统一下单
        $openId = "ofT-Q4qf2xTtbaanGU8T_SGi0Jrc";
        $product_id = '123';

        $config = new WxPayConfig();
        $input = new WxPayUnifiedOrder();
        $input->SetBody("test");
        $input->SetAppid('wx0fb2e2c1fd1db8c7');
        $input->SetAttach("test");
        $input->SetOut_trade_no($config->merchantId . date("YmdHis"));
        $input->SetTotal_fee("1");
        ...
        try {
            ...
            $sign_str = "appId=wx0fb2e2c1fd1db8c7&nonceStr=$result->nonce_str"."&package=prepay_id=$result->prepay_id&signType=MD5&timeStamp=$time&key=$key";
            // echo $sign_str;
            // exit;
