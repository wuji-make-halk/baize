...
<html style="height: 100%;">
		...
		<script type="text/javascript">
			$(function(){
				$("#search").click(function(){
					location.href="/index.php/Test_report/sign_tongji?start="+$("#start").val()+"&end="+$("#end").val()+"&platform="+$("#platform").val()+"&server_id="+$("#server_id").val();
				});
			});
		</script>
	</head>
	...
	<!--右侧边导航栏 start-->
		<!-- <div class="col-md-1"> -->
		  <?php // $this->load->view("admin/admin_sub_menu", ['sign'=>true]); ?>
		<!-- </div> -->
	<!--右侧边导航栏 end-->
		...
		<div class="col-md-11 myborder">
			...
			<h4><?php echo "共 ".$sign_res_num." 人"?></h4>
				<table border="1" cellspacing="0" cellpadding="0" style="width:100%;height:auto;color:#000;background-color:#fff;">
					...
					<?php foreach($sign_res as $sign_one){ $money=0;$pay_num=0;?>
					<tr>
						<td style="padding:5px;"><?php echo date('Y-m-d',$sign_one->create_date);?></td>
						<td style="padding:5px;"><?php echo $sign_one->user_id;?></td>
						<td style="padding:5px;"><?php
						if(@$create_role_res){
							foreach($create_role_res as $create_role_one){
								if($create_role_one->user_id == $sign_one->user_id){
									echo $create_role_one->nickname." —— ".date('Y-m-d',$create_role_one->create_date)."<br/>";
								}
							}
						}
						?></td>
						<td style="padding:5px;"><?php echo $sign_one->platform;?></td>
						<td style="padding:5px;"><?php echo $sign_one->p_uid;?></td>
						<td style="padding:5px;"><?php echo $sign_one->server_id;?></td>
						<td style="padding:5px;"><?php
							if(@$game_order_res){
								foreach($game_order_res as $game_order_one){
									if($sign_one->user_id == $game_order_one->user_id){
