...
<head>
    ...
    <script src='/js/jssdk/jisuiostishentwo_g2b.js?v=<?php echo time(); ?>'></script>
	<script src='https://tfb.lingleigame.com/jb/v1.3/llbridge.min.js'></script>
	...
	html{
		...
		<?php if ($appId == 1291 || $appId == 1292 || $appId == 1345) {
    echo 'background-image: url("/img/login/huawei/jsts2.jpg");';
} elseif ($appId == 1285) {
    echo 'background-image: url("/img/login/huawei/xjl.jpg");';
}?>
	...
	<script type="text/javascript">
		...
		window.addEventListener('load', function(){
			//设置背景操作
			// alert("width: "+document.documentElement.clientHeight+" height: "+document.documentElement.clientWidth);
		...
		function showPreScreen(){
			...
			// frameWidth = window.screen.availWidth;
			// frameHeight = window.screen.availHeight;
			...
			// document.removeEventListener('touchstart', preventDefaultHandler, false);//解除事件处理程序（解除绑定 touchstart）
			 ...
			 document.getElementById('loader').style.width = (frameWidth>frameHeight?frameHeight*0.6:frameWidth)+'px';
 			document.getElementById('loader').style.height = frameHeight+'px';

			document.body.width = (frameWidth>frameHeight?frameHeight*0.6:frameWidth)+'px';
			 ...
			 if (navigator.userAgent.match(/(iPhone)/)) {
    			if ((screen.availHeight == 812) && (screen.availWidth == 375)) {
        			if ((window.innerHeight == "375") && (window.innerWidth == "812")) {// iPhone X Landscape 横屏
							...
							document.getElementById('gbox').style.marginTop="40px";
				        }
				    }
				}
...
<body scroll='no' style='background-color:white;'>
	<div id='gbox' style='position:absolute;overflow:hidden;display:none;width:480px;height:800px'>
		...
		<div id='loading1' class='pa' style='display:none;z-index:200;width:100%;left:0px;top:0px;overflow:hidden;'>
	...
	<div id="loader" style='overflow: hidden;position:absolute;margin:auto;top:0px;left:0px;bottom:0px;right:0px;display:none'>
	...
	<!-- 游戏界面 -->
	<div id='gameDiv' style='overflow: hidden;position: absolute;margin:auto; top: 0px; left: 0px;right:0px;display:none;background-color:black'>
