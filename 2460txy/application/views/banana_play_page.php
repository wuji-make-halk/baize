...
<head>
	...
	iframe.id = "jsurl_mainframe";
	iframe.name = "jsmain";
	iframe.src = 'http://www.qidian777.com/mobile.php?s=/Game/singlegame.html';
	...
	iframe.onload = function () {
		if (!data.user_id) {
			...
			ifr.contentWindow.postMessage(loginparam, 'http://www.qidian777.com');
		}
	};
	document.body.appendChild(iframe);
						if(!param['user_id']){
							...
							//调起运营方登录sdk，并且发送数据
						  	window.addEventListener('message', function(e) {//监听运营方返回消息
							console.log(e);
							// alert(e.data.status);
							...
							if(e.data.event == 'login'&&e.data.status == 1){
          					...
          					var data = param;
          					if(!data.user_id){
            						var userdata = JSON.stringify(param);
            						var ifr = document.querySelector('#jsurl_mainframe');
									// alert(ifr+' ifr');
            						ifr.style.display= "";
            						loginparam = {"event":"login","data":userdata,"status":0};
...
<body scroll='no' style='background-color:white'>
	...
	<!-- <iframe id="jsurl_mainframe" frameborder="0" scrolling="yes"  name="jsmain" src="http://www.qidian777.com/media.php?s=/Game/singlegame.html"
style="top:0px;height:100vh; display: none; visibility: inherit; width: 100vw;max-width:1000px; z-index: 1;overflow: visible;position:absolute;background-repeat:no-repeat;background-size:cover;"></iframe> -->
