...
<head>
    ...
    <script src='/js/jssdk/huawei_g2b.js?v=<?php echo time(); ?>'></script>
	...
	<style type="text/css">
		...
		#gbox {
			<?php if($game->game_father_id == 20012){
				echo "background-image: url(//h5sdk.cdn.zytxgame.com//img/login/huawei/zhtl.jpeg);
					background-repeat:round;";
			}elseif ($game->game_father_id == 20009) {
				echo "background-image: url(//h5sdk.cdn.zytxgame.com//img/login/huawei/xxcz.jpeg);";
			}
			?>
	...
	// 	window.HiSpaceObject.getPlayerId();
	// }
	function setPlayerId( playerId, appId, ts, sign){ // 游戏登录成功
		// alert("华为游戏中心登录成功\n setPlayerId"); // sign 验签....
		// login inner ....

		window.HiSpaceObject.showBuoy();
		huaweiInfo.playerId=playerId;
		huaweiInfo.appId=appId;
		huaweiInfo.ts=ts;
		huaweiInfo.sign=sign;
		var _huaweiLogin = document.getElementById('huaweiLogin');
		_huaweiLogin.style.display='none';
		var param = g2b.getParameters();
		console.log("gameAppId " + gameAppId + " passId " + passId) ;
		param['appId'] = gameAppId;
		param['passId'] = passId;
	...
	function gameInnerlogin(){
		// alert("游戏内部登录成功");

		document.getElementById("t").style.visibility = "hidden"; // 进入游戏...
		// 调用显示华为游戏中心浮标
		// window.HiSpaceObject.showBuoy();
		// alert("开始调起华为快捷支付");
		// window.HiSpaceObject.startPay("");
	 }
	...
	// 支付结果
	function setPayResult(returnCode, userName, orderID, requestId,amount, errMsg, time,sign)
	{
		// alert("支付结果 \n "+returnCode+" |userName "+userName+" |orderID "+orderID+" |requestId "+requestId+" |amount "+amount+" |errMsg "+errMsg+" |time "+time+" |sign "+sign);
		// 校验
	}
	</script>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<title><?php echo $geme->game_name ?></title>
</head>
<body scroll='no' style='background-color:white'>
	<div id='huaweiLogin' style='position: absolute;top: 50%;left: 50%;transform: translate3d(-50%,-50%,0);width: 20%;background: #fff;border-radius: 10px;text-align: center;z-index: 9999;opacity:0.8;' onclick="window.HiSpaceObject.getPlayerId();">
			<div>
				<p>点击登录</p>
			</div>
	</div>
