...
class Kuaida_model extends CI_Model
{
    public $platform = 'kuaida';
    ...
    public function get_order_id()
    {
        ...
        $custom_info = $this->input->get_post('cpOrderId');
    ...
    public function sign_order($game_id = '')
    {
        ...
        // + "&cpgameid=" + cpgameid
        // + "&server=" + param.ext
        // + "&roleid=" + param.cproleid;
        $order_id = $this->input->get('order_id');
        $money = $this->input->get('money');
        $userId = $this->input->get('user_id');
        $goodsName = $this->input->get('goodsName');
        $cpgameid = $this->input->get('cpgameid');
        $srid = $this->input->get('server');
        $roleid = $this->input->get('roleid');
        $key = $this->Game_model->get_key($game_id, 'Paykey');
        $time = time();
        // echo "cpOrderId=$order_id&gameId=$cpgameid&goodsId=$money&goodsName=$goodsName&money=$money&role=$roleid&server=$srid&time=$time&uid=$userId&key=$key";
        $sign = MD5("cpOrderId=$order_id&gameId=$cpgameid&goodsId=$money&goodsName=$goodsName&money=$money&role=$roleid&server=$srid&time=$time&uid=$userId&key=$key");
