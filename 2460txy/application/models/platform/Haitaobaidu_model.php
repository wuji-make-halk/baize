...
class Haitaobaidu_model extends CI_Model
{
    public $platform = 'haitaobaidu';
    ...
    public function login()
    {
        ...
        $url = "http://h5.allugame.com/index.php/api/get_user_info?token=$token&uid=$user_id";
        ...
        if ($response->c == 0) {
            $allu_user = $response->d;
            $condition = array(
                'p_uid' => $allu_user->user_id.'',
            ...
            if (!$user) {
                $user = array(
                    'platform' => $this->platform,
                    'p_uid' => $allu_user->user_id,
                    'nickname' => $allu_user->nickname,
                    'avatar' => $allu_user->headimgurl,
                    'create_date' => time(),
                );
    ...
    public function sign_order($game_id = '')
    {
        ...
        if ($game_id != 1000) {
            ...
            log_message('debug', $this->platform.' aibei info is '.json_encode($transdata));
                ...
                $res = json_decode(urldecode($ps['transdata']));
                    ...
                    if (!isset($res->code)) {
                        ...
                        $h5data = array(
                            'tid' => $transid,
                            'app' => '3012208265',
                            'url_r' => 'http://h5.allugame.com/index.php/game/redirect/'.$_go_back_game_id,
                            'url_h' => 'http://h5.allugame.com/index.php/game/redirect/'.$_go_back_game_id,
                            'ptype'=>'403'
                        );
