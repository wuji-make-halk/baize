...
class Shunw_model extends CI_Model
{
    public $app_key = '';
    public $platform = 'shunw';
    public $pay_type='';
    ...
    public function login($game_id)
    {
        ...
        $sign=md5("guid=$g_uid&time=$time&$this->app_key");
        if (!$g_uid) {
            return false;
        }
        $condition = array(
    ...
    public function get_order_id()
    {
        ...
        $order_id = $this->input->get_post('itemId');
        // $account = $this->input->get_post('guid');
        // $this->session->set_userdata('account', $account);
        $money = $this->input->get_post('money');
        // $orderId = $this->input->get_post('orderNo');
        //
        // $this->cache->save('orderId', $orderId);
        // $this->cache->save('itemId', $order_id);
        //
        ...
        // if ($sign!=$pay_sign) {
        //     log_message('error', $this->platform.' sign is not match, sign is'.$sign.' pay_sign is:'.$pay_sign);
        //     return;
        // }
        //
        // if (!$order_id) {
        //     log_message('error', $this->platform.' order_id is not match, order_id is'.$order_id);
        //     return;
        // }
        //
        // $coins = $this->input->get_post('coins');
    ...
    public function sign_order($game_id = '')
    {
        ...
        $this->app_key = $this->Game_model->get_key($game_id, 'appkey');
        $time = time();
        $sign = md5($guid.$serveridx.$time.$money.$itemid.$this->app_key);
        // $sign = md5($guid.$serveridx.$time.'256'.$money.$itemid.$this->app_key);
