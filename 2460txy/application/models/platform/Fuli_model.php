...
class Fuli_model extends CI_Model
{
    public $platform = 'fuli';
    ...
    public function sign_order($game_id = '')
    {
        ...
        $this->load->model('Game_order_model');
        $response = $this->Game_order_model->get_one_by_condition($condition);

        if ($response) {
            $data = $response;
            $data->handler_name = '';
            $data->handler_status = 1;
            $data->nickname = $nickname;

            $this->load->model('Game_order_fuli_model');
            $id = $this->Game_order_fuli_model->add($data);
            if ($id) {
                return $id;
            } else {
                return false;
