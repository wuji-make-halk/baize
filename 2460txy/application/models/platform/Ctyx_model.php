...
class Ctyx_model extends CI_Model
{
    public $platform = 'ctyx';
    public $appkey = '';
    public $appid = 0;
    ...
    public function login($game_id)
    {
        ...
        $this->appkey = $this->Game_model->get_key($game_id, 'appkey');

        $my_sign = md5('appId='.$this->appid.'&userId='.$userId.$this->appkey);
        if ($my_sign != $sign) {
            log_message('error', 'Ctyx sign error');
        ...
        if (!$user) {
            $url = 'http://h5.t4game.com/user/info?userId='.$userId.'&appId='.$this->appId.'&sign='.$my_sign;
    ...
    {
        // $order_id = $this->input->post('extInfo');
        $userId = $this->input->post('userId');
        $orderId = $this->input->post('orderId');//平台方的订单号
        $order_id = $this->input->post('cpCode');//CP订单号
    ...
    public function sign_order($game_id)
    {
        ...
        if (!$orderId || !$subject || !$money || !$userId || !$buyAmount) {
            return false;
        }
