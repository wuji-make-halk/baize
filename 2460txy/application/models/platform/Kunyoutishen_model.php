...
class Kunyoutishen_model extends CI_Model
{
    public $platform = 'kunyoutishen';
    ...
    public function login($game_id)
    {
        ...
        $url = "http://sapi.kunyouxinxi.cn:9001/sdk/token/verify";
        $request = $this->Curl_model->curl_post($url, $data);
    ...
    public function get_order_id()
    {
        ...
        // log_message('debug', $this->platform . ' order ' . json_encode($request));
        // $array = parse_str(json_encode($request));

        $str = 'www.a.com?' . urldecode($request);
        parse_str(parse_url($str)['query'], $parr);
        $data = json_decode($parr['data']);
