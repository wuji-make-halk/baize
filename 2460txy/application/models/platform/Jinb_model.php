...
class Jinb_model extends CI_Model
{
    public $platform = 'jinb';
    ...
    public function login($game_id)
    {
        ...
        log_message('debug',$this->platform.' sign str '.$pairs.$this->key.' sha1 '.$sign);
        ...
        if ($content) {
            ...
            if (isset($response_obj) && $response_obj->code == 0) {
                ...
                // if ($this->cache->redis->is_supported()) {
                    $this->cache->save($user['user_id'].'_token', md5($user['user_id'].$user['platform'].time()), 300);
                // }

                // log_message('debug', $this->platform.' openKey is '.$_openKey.' openId is '.$user['user_id'] );
                return $user['user_id'];
            }
        }
    }
    ...
    public function trun_to_game($game_id)
    {
        ...
        if (!$openKey) {
            log_message('debug', $this->platform.' openKey is null, id is: '.$openId);
            echo '_error';
            return ;
        }
        $appId = $game_id;
    ...
    public function focus()
    {
        ...
        if ($content) {
            ...
            if (isset($response_obj) && $response_obj->code == 0) {
                ...
                return array("status" =>$response_obj->message);
            }
        }
