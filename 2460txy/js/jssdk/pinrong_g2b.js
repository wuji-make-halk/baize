var g2b = function () {
    ...
    var login = function (res, cb) {
        ...
        if (passId == "pinrong") {
            loadScript("//soeasysdk.com/soeasysr/zm_engine_v2.js", function () {
                //sdk 加载完毕回调
                window.zmInitSucc = function () {
                    ...
                    ZmSdk.getInstance().init(function (data) {
                        //初始化成功之后调用其他 sdk 能力
                        //如 获取用户信息、支付、角色上报、设置分享信息、分享等...
                        if (data.retcode === "0") {
                            var userinfo = ZmSdk.getInstance().getUserInfo();
                            console.log(userinfo);

                            var user_id = userinfo.userdata.uid;
                            pf_params.sdkindx = userinfo.common.sdkindx;
                            pf_params.uid = user_id;
