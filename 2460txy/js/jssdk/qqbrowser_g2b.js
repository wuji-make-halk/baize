    ...
    var login = function(res, cb) {
        ...
        if (passId == "qqbrowser") {
            ...
            $(document).ready(function() {
                sdk = window.sdk = window.browser.x5gameplayer;
                //配置SDK属性,向SDK注册登录的回调
                // console.log(JSON.stringify(sdk));
                sdk.config && sdk.config({
                    loginCallBack: 'http://h5sdk.zytxgame.com/index.php/enter/play/qqbrowser/1126'
                });
                // console.log(JSON.stringify(sdk));
                // alert(JSON.stringify(sdk));
                // console.log(JSON.stringify(sdk.config));
            });
            var options = {
                appid: '8493315297'
            };
            browser.x5gameplayer.getAvailableLoginType(options, function(callback) {
                // alert(JSON.stringify(callback));
                // console.log(callback.loginTypes[0].loginType);
                if (callback.result == 0) {
                    var _getAppSigUrl = 'http://h5sdk.zytxgame.com/index.php/Qqb_api/GetAppSig';
                    getDataXHR(_getAppSigUrl, function(response) {
                        if (response.c == 0) {
                            var option = {
                                appid: '8493315297',
                                appsig: response.d.appSig,
                                appsigData: response.d.appsigData,
                                loginType: callback.loginTypes[0].loginType,
                            }
                            // console.log(option);
                            // alert(JSON.stringify(option));
                            browser.x5gameplayer.login(option, function(callback) {
                                alert(JSON.stringify(callback));
                            });
                        }
                    });
