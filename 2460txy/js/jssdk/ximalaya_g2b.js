var g2b = function() {
    ...
    var login = function(res, cb) {
        ...
        if (passId == "ximalaya") {
            loadScript("http://s1.xmcdn.com/lib/h5ol/last/js/gameInterface.js", function() {
                getDataXHR("/index.php/api/focus/" + passId + "/" + appId, function(_res) {
                    if (_res.c == 0) {
                        var xmlyt = getUrlParam('xmlyt');
                        // console.log(xmlyt);
                        var obj = {
                            'client_id': _res.d.client_id,
                            'appSecret': _res.d.appSecre,
                            'redirect_uri': 'http://h5sdk.zytxgame.com/index.php/enter/play/ximalaya/1187?xmlyt=1',
                            'game_id': _res.d.game_id,
                            'secret_key': _res.d.secret_key,
                            'sign_type': 'RSA',
                            'success': function(response) {
                                // window.top.location.href = 'http://h5sdk.zytxgame.com/index.php/enter/play/ximalaya/1187?xmlyt=1&uid='+res.uid;
                                // console.log(JSON.stringify(response));
                                if (response.code == 0) {
                                    // console.log(response);
                                    // console.log(response.data.out_user_id);
                                    // console.log(response.access_token);
                                    res['uid'] = response.data.out_user_id;
