var g2b = function() {
    ...
    var login = function(res, cb) {
        ...
        golappid = appId;
        golpassid = passId;
        if (passId == "langaohuyu") {

            // _cplogin();


            // console.log(getUserInfo());
            ticketDemo = getParameter("ticket");
            getUserInfo(res, cb);
    ...
    var init = function(data) {
        ...
        // }
        // if (data.test) {
        loadScript("//" + location.host + "/js/platform.js", function() {
            loadScript("//" + location.host + "/js/platforms/PLATFORM_" + passType + ".js", function() {
        ...
        // } else {//     loadScript(passHost + "/js/platform.js", function() {
        //         loadScript(passHost + "/js/platforms/PLATFORM_" + passType + ".js", function() {
        //             pf_obj = new pf(g2b, shareInfo, pf_params, {
        //                 appId: appid,
        //                 passId: passId,
        //                 gameId: gameId
    ...
    var host = "http://xiaoma.dbdna.com";
    var appId = 'c7912b7f-a256-4813-9b42-468bd2cf795a'; //互娱自己获取或询问用户认证中心工作人员
    //若进入页面前用户已登录，进入页面时会作为参数附带 ticket
    ...
    //获取当前用户信息——模拟需要登录才能完成的功能，
    function getUserInfo(res, cb) {
        var that = this;
        $.ajax({
            type: 'get',
            url: host + "/usercenter/sso/getUserInfo", //互娱平台调用时应加上域名http://xiaoma.dndna.com/
            data: {
                "appPurpose": appId
            },
            xhrFields: {
            ...
            crossDomain: false, //必选，否则无法判断请求是否是ajax
            headers: {
                "ticket": ticketDemo
            },
            complete: function(xhr, data) {
                if (xhr.status == 401) { //武汉公交未登录情况被标注为401
                    _cplogin();
                } else if (xhr.status == 402) { //其他需调用用户中心本地登录页面的被标注为402
                    window.location.href = host + "/ssoStatic/signIn.html" + "?appPurpose=" + appId + "&redirectUrl=" + window.location.href;
                } else if (xhr.status == 200) { //用户已登录
                    //从header中获取新的ticket
                    console.log("从header中获取ticket:" + xhr.getResponseHeader("ticket"));
                    ticket = xhr.getResponseHeader("ticket");

                    //localStorage.setItem("ticket", xhr.getResponseHeader("ticket"));
                }
            },
    ...
    //武汉公交在登录完成后会回掉此方法
    function mobileOnComplete(data) {
        var code = data.code;
        var appSource = data.appSource;

        //登录成功后武汉公交返回code和appSource,用来校验code获取ticket.
        $.ajax({
            url: host + "/usercenter/sso/putCode",
            data: {
                "code": code,
                "appSource": appSource,
                "appPurpose": appId
            ...
            success: function(res, textStatus, request) {
                //打印ticket
                $("#others").html(request.getResponseHeader("ticket"));
                if (res.success) {
                    var ticket = request.getResponseHeader("ticket");
                    $("#ticket").html("ticket是：" + ticket);
                } else {
                    $("#ticket").html(res);
                }
            }
