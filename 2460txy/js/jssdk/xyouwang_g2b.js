        ...
        if (passId == "xyouwang") {
            ...
            getDataXHR(focusUrl, function (res) {
                if (res.c == 0) {
                    ...
                    xy_sdk.config(gameKey, function () {
                        //充值成功触发回调
                        console.log("充值成功");
                    }, function () {
                        //分享成功触发回调
                        // alert('ok');
                        that.postMessage('msg_share_cb', true);
                    }, function (data) {
                        /*
                         *微信公众号关注状态异步通知（被动接收状态通知）:
                         *该状态通知与getWxSubscribe接口的差别在于被动接收状态通知与主动获取状态
                         *接入方可在两种状态获取方式中自行选择
                         */
                        console.log('微信公众号关注状态：' + data.state); //state：0未关注，1已关注，2已取消关注
                        window.focusStage = data.state;
                        // pf_obj.checkFocus();
                    });


                    xy_sdk.getUserInfo(function (data) {
                        console.log(data);
