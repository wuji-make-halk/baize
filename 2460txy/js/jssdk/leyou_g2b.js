var g2b = function() {
    ...
    var login = function(res, cb) {
        ...
        if (passId == "leyou") {
            var user_info;
            console.log('get in leyou');
            loadScript("http://api2.57533.com/h5/sdk.js", function() {
                console.log("g2b login");
                window.gameSDK.init(
                    'jSid5P20170613493', //App-Key
                    'febf7fb1e42ded00cdaec0e6ebe58842', //App-Secret
                    'GameCustomParameters',
                    function(userInfo) {
                        res['uid'] = userInfo.uuid;
                        url = url + object2search(res);
                        //callback userInfo(  uuid)
