var g2b = function() {
    ...
    var login = function(res, cb) {
        ...
        if (passId == "vivo") {
            VivoGame.call('isSupportH5Game', {
                "appid": "036437a75c7cd1171171f1538bcac804"
            }, function(res) {
                console.log(res);
            });
            VivoGame.call("getEarInfo",  function(data) { 
                var obj = JSON.parse(data); 
                shipeiIsSupport = obj.result; //是否支持刘海屏
 
                shipeiWidth = obj.width; // 刘海屏宽度
 
                shipeiHeight = obj.height; // 刘海屏高度
 
            } );
            VivoGame.call("login", function(data) {
                var obj = JSON.parse(data);
                uid = obj.openid;
                res['openid'] = uid;
                pf_params = res;
                ...
                getDataXHR(url, function(data) {
                    if (data.c == 0) {
                        ...
                        if (shipeiIsSupport) {
                            gameFrame.height = shipeiHeight * 0.8;
                            gameFrame.shipeiHeight = shipeiHeight * 0.6;
                        }
