        ...
        if (passId == "hiwan") {
            loadScript("http://g.hwwh5.com/js/hww_sdk_sub.js", function () {
                Hwwsdk.onuserinfo(function (jsonInfo) {
                    ...
                    var callBackInfo = jsonInfo.regfrom;
                    Hwwsdk.onshare(function (info) {
                        // info.status 1-分享成功  其他-分享失败
                        if (info.status == 1) {
                            postMessage(MESSAGES.SHARE_CALLBACK, true);
                        } else {
                            postMessage(MESSAGES.SHARE_CALLBACK, false);
                    ...
                    res['userAccount'] = jsonInfo.userAccount;
                    res['nickname'] = encodeURIComponent(jsonInfo.nickname);
                    res['sex'] = jsonInfo.sex;
                    res['headImgUrl'] = encodeURIComponent(jsonInfo.headImgUrl);
