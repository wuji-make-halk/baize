var g2b = function () {
    ...
    var login = function (res, cb) {
        ...
        if (passId == "kaiersasi") {
            loadScript("http://soeasysdk.com/soeasysr/zm_engine_v2.js", function () {

                //sdk加载完毕回调
                window.zmInitSucc = function () {
                    console.log("cp start work"); //TODO 必须在这里或者该方法调用之后进行sdk调用
                    ZmSdk.getInstance().init(function (data) { //初始化成功之后调用其他sdk能力如获取用户信息、支付、角色上报、设置分///享信息、分享等...掌盟SoEasy SDK开发者帮助文档//示例：
                        if (data.retcode === "0") {
                            var userinfo = ZmSdk.getInstance().getUserInfo();
                            console.log(userinfo);
                            res['uid'] = userinfo.userdata.uid;
                        ...
                        } else if (data.retcode === "1") { //初始化失败处理
                        }
                    });
                }
