var g2b = function () {
    ...
    var login = function (res, cb) {
        ...
        if (passId == "yinli") {
            var focusUrl = "/index.php/api/focus/" + passId + "/" + appId;
            getDataXHR(focusUrl, function (res) {
                if (res.c == 0) {
                    var gameKey = res.d;
                    var kukuJSURL = 'https://game.kuku168.cn/public/js/kuku_js_sdk.min.js?timestamp=' + new Date().getTime();
                    loadScript(kukuJSURL, function () {
                        ylsdk = window.KUKU_JS_SDK;
                        ylsdk.config(gameKey, function () {
                            //分享回调通知设置是否成功
                            //根据具体业务，做你自己的事情
                            console.log('分享回调', args)
                            postMessage(this.g2b.MESSAGES.SHARE_CALLBACK, true);
                            // ylsdk.toast(3, msg || '分享信息已经设置好了')
                            result.innerText = JSON.stringify(args)
                        }, function () {});
                        ylsdk.getUserInfo(function (data) {
                            var uid = data.uid;
                            console.log("uid: " + uid);
                            //deal with success
                            res['uid'] = uid;
            ...
            // switch (appId) {
            //     case 1380:
            //     var gameKey = "GK-20180905161623-1873";
            //         break;
            //     case 1385:
            //     var gameKey = "GK-20181010175328-1575";
            //         break;
            //     default:
            //         break;
            // }
