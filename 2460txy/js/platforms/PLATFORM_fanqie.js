...
pf.prototype.pay = function (amount, orderData) {
    ...
    this.g2b.createPay({
        ...
        var url = "//" + location.host + "/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId +
            "?account=" + cpAccount +
            "&amount=" + readl_money +
            "&description=" + param.goodsName +
            "&game_order_id=" + generate_order_id +
            "&name=" + param.goodsName +
            "&openid=" + cpOpenid +
            "&unit_price=" + readl_money +
            "&token=" + cpToken;

        this.g2b.getDataXHR(url, function (response) {
            if (response.c == 0) {
                // var res_url = 'https://game.qianyuanclub.cn/pay?data=' + response.d;
                ...
                var jump_to = 'https://game.qianyuanclub.cn/pay?data=' + response.d;
