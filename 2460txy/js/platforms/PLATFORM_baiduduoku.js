...
pf.prototype.init = function() {
    ...
    cpToken = this.pf_params.accessToken
...
pf.prototype.pay = function(amount, orderData) {
    ...
    this.g2b.createPay({
        ...
        var url = "//" + location.host + "/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId + "?order_id=" + generate_order_id +
            "&money=" + readl_money + "&openId=" + orderData.openId + "&userId=" + userId + "&goodsName=" + param.goodsName + "&token=" + cpToken;

        this.g2b.getDataXHR(url, function(response) {
            if (response.c == 0) {
                var payInfo = JSON.stringify(response.d);
                // alert(payInfo);
                dk_sdk.pay(JSON.parse(payInfo));
            }
