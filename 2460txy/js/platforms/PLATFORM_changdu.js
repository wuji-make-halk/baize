...
pf.prototype.init = function() {
    ...
    var that = this;
    this.g2b.loadScript('http://img.51changdu.com/sharp/H5BookStore/js/readerPaygame.min.js', function() {
        cpUid = that.pf_params.uid;
        cpAction = that.pf_params.action;
  ...
  param.cproleid = orderData.cproleid;
    ...
    this.g2b.createPay({
        ...
        var url = "http://h5sdk.zytxgame.com/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId + "?order_id=" + generate_order_id +
            "&money=" + readl_money + "&uid=" + userId + "&serverId=" + param.ext;

        this.g2b.getDataXHR(url, function(response) {
            if (response.c == 0) {

                h5.paygame( response.d.game_id,userId, readl_money, readl_money, param.ext, response.d.time, generate_order_id, response.d.sign, cpAction);
            }
...
pf.prototype.reportData = function(data) {
    ...
    if (data.action == 'enterGame') {
        ...
        // this.g2b.getDataXHR(_update, function(response) {
        //     if(response.c == 0 ){
        //         var _cp ="http://img.andreader.com/worldcup/thegame/notify.ashx?op=createrole&gameid="+response.d.id+"&uid="+cpUid+"&serverid="+srvid+"&createtime="+response.d.time+"&sign="+response.d.sign;
        //         that.g2b.getDataXHR(_cp, function(response) {
        ...
        var _update = "http://h5sdk.zytxgame.com/index.php/api/focus/" + that.passData.passId + "/" + that.passData.appId + "?srvid=" + srvid + "&uid=" + cpUid;
        this.g2b.getDataXHR(_update, function(response) {});
