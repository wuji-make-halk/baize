...
pf.prototype.pay = function (amount, orderData) {
    ...
    this.g2b.createPay({
        ...
        this.g2b.getDataXHR("//" + location.host + "/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId + "?amount=" + readl_money + '&server=' + param.ext + '&uid=' + user_id + '&order_id=' + generate_order_id + '&token=' + token, function (response) {
            if (response.c == 0) {
                var sign = response.d.sign;
                var time = response.d.time;
                var appid = response.d.appid;
                // var data = response.d.data;
                // http://h5sdk.btgame99.com/index.php/pay/payorder/
                // http://h5sdk-hk.btgame01.com/index.php/pay/payorder
                var payUrl = "http://h5sdk.btgame99.com/index.php/pay/payorder/?appid=" + appid + "&uid=" + user_id + "&token=" + token + "&amount=" + readl_money + "&serverid=" + param.ext + "&extra_orderno=" + generate_order_id + "&time=" + time + "&sign=" + sign;
