...
pf.prototype.init = function() {
	...
	this.g2b.getDataXHR('http://h5sdk.zytxgame.com/index.php/api/focus/kemeng/1060', function(response) {
		if (response.c == 0) {
			u = navigator.userAgent;
			isAndroid = u.indexOf('Android') > -1 || u.indexOf('Adr') > -1; //android终端
			isiOS = !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端

			ios_or_android = response.d.pf;
			if (isiOS) {
				ios_or_android = 102;
			} else {
				ios_or_android = response.d.pf;
			}
			console.log(ios_or_android);
...
pf.prototype.reportData = function(data) {
	...
	if (data.action == 'enterGame') {
		...
		var url = "//h5sdk.zytxgame.com/index.php/api/sign_collect/kemeng/" + this.passData.appId + "?roleid=" + roleid + "&srvid=" + srvid;
		this.g2b.getDataXHR(url, function(response) {});
	}
};
