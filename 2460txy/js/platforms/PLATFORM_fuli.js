...
pf.prototype.pay = function (amount, orderData) {
    ...
    this.g2b.createPay({
        ...
        console.log('createPay', JSON.stringify(res));
        var generate_order_id = res.d.order_id;
        var userId = res.d.userId;
        var readl_money = amount; // for test
        var show_money = readl_money / 100;

        var url = "//" + location.host + "/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId + "?order_id=" + generate_order_id + "&money=" + readl_money + "&user_id=" + userId+"&nickname="+cpnickname;
        this.g2b.getDataXHR(url, function (response) {
            console.log(response);
            if (response.c == 0) {
                alert('您创建了 ' + show_money + ' 元的充值订单。充值订单需审批，需要 1 - 2 个工作日。具体详情联系客服！');
            }
        });
