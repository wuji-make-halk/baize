...
pf.prototype.pay = function (amount, orderData) {
    ...
    this.g2b.createPay({
        ...
        that.g2b.getDataXHR(url, function (response) {
            if (response.c == 0) {
                var _url = `http://appsdk.gamei.cn/webldqmxh5/pay_confirm_new.php?serv_id=${param.ext}&money=${readl_money}&coin=${readl_money}&serv_name=${param.ext}&player_id=${orderData.openId}&usr_name=${_nickname}&content=${param.goodsName}
                    &time=${response.d.time}&order_id=${generate_order_id}&platform=${generate_order_id}&sign=${response.d.sign}`;
                // iframePay.open(_url, function () {
                //     console.log("onclose");
                // });
