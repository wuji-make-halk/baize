  ...
  param.cproleid = orderData.cproleid;
    ...
    this.g2b.createPay({
        ...
        var url = "http://h5sdk.zytxgame.com/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId +
            "?order_id=" + generate_order_id + "&money=" + readl_money +
            "&openId=" + orderData.openId + "&userId=" + userId +
            "&goodsName=currency";
        this.g2b.getDataXHR(url, function(response) {
            if (response.c == 0) {
                var payData = JSON.parse(response.d.params);
                var params = {
                    productName: 'currency',
                    productDes: 'currency',
                    productPrice: readl_money,
                    vivoSignature: payData.accessKey,
                    appId: response.d.appid,
                    transNo: payData.orderNumber,
                    uid: uid,
                    extInfo:generate_order_id
                }
