...
pf.prototype.init = function() {
    ...
    channelExt = this.pf_params.channelExt;
  ...
  param.cproleid = orderData.cproleid;
    ...
    this.g2b.createPay({
        ...
        var url = "http://h5sdk.zytxgame.com/index.php/api/sign_order/" + that.passData.passId + "/" + that.passData.appId + "?order_id=" + generate_order_id +
            "&money=" + readl_money + "&openId=" + orderData.openId + "&channelExt=" + channelExt + "&userId=" + cpuid + "&goodsName=" + encodeURIComponent(param.goodsName);

        this.g2b.getDataXHR(url, function(response) {
            if (response.c == 0) {
                $("#dhwgame").append('<iframe id="payurl_mainframe"></iframe>');
                // var obj = {};
                // var paydata = JSON.stringify(obj);
                var paydata = JSON.stringify(response.d.par);
                var payifr = document.querySelector('#payurl_mainframe');
                var uurl = response.d.url;
                //判断当前模块
                console.log(uurl);
                if (IsPC()) {
                    console.log('1');
